...
//        //System.out.println(test.execute("let $items := 0 return count(for $x in doc(\"DB/books.xml\")/all_books/book where $x/cover=\"soft\" return $x/cover)"));
    ...
    public static String task3e(String author) throws XQException {
        String query = "let $author := \""+author+"\" " +
                "for $x in doc(\"DB/books.xml\")/all_books/book where $x/author = $author return $x/title";
        return getData(query);
    }

    public static String task3f(String publisher) throws XQException {
        String query = "let $publisher := \""+publisher+"\" " +
                "for $x in doc(\"DB/books.xml\")/all_books/book where $x/publisher = $publisher return $x/title";
        return getData(query);
    }

    public static String task3g(String genre) throws XQException {
        String query = "let $genre := \""+genre+"\" " +
                "for $x in doc(\"DB/books.xml\")/all_books/book where $x/genre = $genre return $x/title";
        return getData(query);
    }

    public static String task3h(int from, int to) throws XQException {
        String query = "let $from := \""+from+"\", $to :=  \""+ to+"\"" +
                "for $x in doc(\"DB/books.xml\")/all_books/book where $x/year > $from and $x/year < $to order by $x/year return $x/title";
        return getData(query);
    }

    public static String task3i(int priceFrom, int priceTo) throws XQException {
        String query = "let $priceFrom := \""+priceFrom+"\", $priceTo :=  \""+ priceTo+"\"" +
                "for $x in doc(\"DB/books.xml\")/all_books/book where $x/price > $priceFrom and $x/price < $priceTo order by $x/price return $x/title";
        return getData(query);
    }
    ...
    public static String task3j(String author) throws XQException {
        String query = "let $author := \""+author+"\" " +
                "return " +
                "(count(for $x in doc(\"DB/books.xml\")/all_books/book where $x/author = $author return $x/author)," +
                "sum(for $x in doc(\"DB/books.xml\")/all_books/book where $x/author = $author return $x/price))";
        return getData(query);
    }

    public static String task3k(String cover) throws XQException {
        String query = "let $cover := \""+cover+"\" " +
                "let $items := 0 return count(for $x in doc(\"DB/books.xml\")/all_books/book where $x/cover = $cover return $x/cover)";
        return getData(query);
    }
